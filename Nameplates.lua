...
local GetUnitClassToken = ns.GetUnitClassToken
local GetClassColor = ns.GetClassColor
local GetUnitClassColor = ns.GetUnitClassColor
local GetPlayerPowerOrder = ns.GetPlayerPowerOrder
    ...
    for unit, myPlate in pairs(ns.unitToPlate or {}) do
        if myPlate then
            ...
            if myPlate.heroPowerBars then
                for _, bar in ipairs(myPlate.heroPowerBars) do
                    bar._lastWidth = nil
                    bar._lastHeight = nil
                end
            end
...
do
    local playerClass = ns.GetPlayerClassToken and ns.GetPlayerClassToken() or select(2, UnitClass("player"))
...
local function PositionHeroPowerBars(myPlate)
    ...
    for i, bar in ipairs(myPlate.heroPowerBars) do
        if bar then
            bar:ClearAllPoints()
...
local function EnsureHeroPowerBars(myPlate)
    ...
    local order = GetPlayerPowerOrder()
...
local function UpdatePowerBar(myPlate, unit)
    local multiPowerOrder = GetPlayerPowerOrder()

    if multiPowerOrder then
...
do
    local pClass = ns.GetPlayerClassToken and ns.GetPlayerClassToken() or select(2, UnitClass("player"))
    ns.playerClassForDebuffs = ns.ResolveClassToken and ns.ResolveClassToken(pClass) or pClass
end
...
function ns:UpdatePersonalBorder()
    ...
    if personalPlateRef.heroPowerBars then
        for i, bar in ipairs(personalPlateRef.heroPowerBars) do
            heroBorders[i] = bar.border
        end
    end
    ...
    local function ShowAllBorders(r, g, b)
        ...
        for i, border in ipairs(heroBorders) do
            if border then
    ...
    local function HideAllBorders()
        ...
        for _, border in ipairs(heroBorders) do
            border:Hide()
        end
    end
...
function ns:FullPlateUpdate(myPlate, unit)
    ...
    local class = isPlayer and GetUnitClassToken(unit) or nil
